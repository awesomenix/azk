...
func (cr clusterResource) List(w http.ResponseWriter, r *http.Request) {
	...
	err = crClient.List(context.Background(), clusterItemList)
	if err != nil {
...
func Events(w http.ResponseWriter, r *http.Request) {
	...
	err = bClient.List(context.Background(), eventList)
	if err != nil {
	...
	for _, eventItem := range eventList.Items {
		...
		events = append(events, eventListType{
			Name:      eventItem.Name,
	...
	eventListJSON, err := json.Marshal(events)
	if err != nil {
		http.Error(w, fmt.Sprintf("Failed to fetch json %v", err), 400)
		return
	}
